...
static int sunxi_spi_parse_pins(struct udevice *bus)
{
	...
	ret = sunxi_gpio_setup_dt_pins(gd->fdt_blob, bus->node.of_offset, NULL,
		pin_func);
...
static void sunxi_spi_enable_clock(struct udevice *bus)
{
	...
	setbits_le32(&ccm->ahb_gate0, (1 << 20));
...
static void sunxi_spi_disable_clock(void)
{
	...
	clrbits_le32(&ccm->ahb_gate0, (1 << 20));
...
static void sunxi_spi_write(struct udevice *dev, size_t nbytes)
{
	struct udevice *bus = dev->parent;
	struct sunxi_spi_priv *priv = dev_get_priv(bus);
	size_t i;
