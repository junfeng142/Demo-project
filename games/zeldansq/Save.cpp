...
void Save::load() {
    ostringstream im;
    im << id;
    ifstream f(("/mnt/FunKey/.zelda-nsq/znq" + im.str() + ".dat").c_str(), ios::in | ios::binary);
    if(!f.is_open()) {
        return;
    }
...
void Save::save(bool endGame) {
    ostringstream im;
    im << id;
    ofstream f(("/mnt/FunKey/.zelda-nsq/znq" + im.str() + ".dat").c_str(), ios::out | ios::binary);
...
void Save::erase() {
    ostringstream im;
    im << id;
    remove(("/mnt/FunKey/.zelda-nsq/znq" + im.str() + ".dat").c_str());
    init();
    loaded = false;
}
